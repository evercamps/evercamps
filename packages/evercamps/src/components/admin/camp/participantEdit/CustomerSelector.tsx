...
interface Props {
  ...
  onUnSelect: (customer: Customer) => Promise<void>;
  selectedChecker: (customer: Customer) => boolean;
  closeModal: () => void;
  disableSelection?: boolean;
}

function CustomerSelector({ onSelect, onUnSelect, selectedChecker, closeModal, disableSelection = false }: Props) {
  const limit = 10;
  ...
  const unSelectCustomer = async (customer: Customer) => {
    try {
          ...
          <div>
            ...
            {!fetching && data && !loading && (
              <div className="divide-y">
                ...
                {data.customers.items.map((customer: Customer) => (
                  <div
