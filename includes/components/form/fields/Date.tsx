...
function Date({ ref, name, value, label, onChange, error, suffix, prefix, placeholder, instruction }: DateProps) {
  const localRef = React.useRef<HTMLInputElement>(null);

  React.useEffect(() => {
    if (!localRef.current) return;
    const instance = flatpickr(localRef.current, { enableTime: false });
